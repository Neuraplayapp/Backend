...
const TEST_CONFIG = {
  ...
  memoryTestCases: [
    // Personal Information for LLM Context Awareness
    ...
    { key: 'pet_breed', value: 'Persian cat, very fluffy and loves to sleep', category: 'personal' },
    { key: 'birthday', value: 'March 15th', category: 'event' },
    { key: 'hometown', value: 'Seattle, Washington', category: 'location' },
    { key: 'current_location', value: 'New York City', category: 'location' },
    { key: 'favorite_food', value: 'Pizza and sushi', category: 'preference' },
    { key: 'hobbies', value: 'Reading sci-fi novels, playing guitar, hiking', category: 'preference' },
    { key: 'career_goal', value: 'Wants to become a data scientist', category: 'goal' },
    { key: 'learning_challenges', value: 'Struggles with time management but excels in visual learning', category: 'learning' },
    { key: 'math_level', value: 'Struggling with algebra but confident in arithmetic', category: 'learning' },
    { key: 'preferred_weather', value: 'Likes sunny days around 75°F', category: 'preference' },
    { key: 'canvas_preferences', value: 'Prefers bright colors and clean layouts', category: 'preference' }
  ],
  
  // Enhanced test categories
  weatherTestCases: [
    { location: 'New York City', preferences: { units: 'imperial', temp_alerts: true } },
    { location: 'Seattle', preferences: { units: 'metric', rain_alerts: true } },
    { location: 'Los Angeles', preferences: { units: 'imperial', uv_alerts: true } }
  ],
  
  canvasTestCases: [
    { canvasId: 'math_workspace', state: { tools: ['pen', 'eraser'], zoom: 1.2 }, workingMemory: { currentProblem: 'algebra_1' } },
    { canvasId: 'science_notes', state: { tools: ['highlighter', 'text'], zoom: 1.0 }, workingMemory: { topic: 'chemistry_basics' } },
    { canvasId: 'art_project', state: { tools: ['brush', 'color_picker'], zoom: 1.5 }, workingMemory: { style: 'abstract' } }
  ],
  
  assistantTestCases: [
    { type: 'small', preferences: { voice_enabled: false, quick_responses: true }, contextData: { session_type: 'quick_help' } },
    { type: 'full', preferences: { voice_enabled: true, detailed_responses: true }, contextData: { session_type: 'deep_learning' } }
  ]
};
...
async function runComprehensiveMemoryTests() {
  ...
  try {
    ...
    // Phase 4: Component-Specific Memory Tests
    await testWeatherMemorySystem();
    await testCanvasMemorySystem();
    await testAssistantMemorySystem();
    
    // Phase 5: LLM Contextual Recall Tests
    await testPersonalContextualRecall();
    await testPreferencesAndHobbiesRecall();
    await testLocationAndSpatialMemory();
    
    // Phase 6: Enhanced Advanced Features
    await testSemanticSearch();
    await testVectorSearchIntegration();
    await testMemoryRanking();
    await testCognitiveProgression();
    
    // Phase 7: Enhanced Cross-Chat Features
    await testCrossChatPersistenceAdvanced();
    await testCrossChatContextTransfer();
    await testAssistantTransferMemory();
 ...
 * 🔧 PHASE 4: COMPONENT-SPECIFIC MEMORY TESTS
 */
async function testWeatherMemorySystem() {
  const testName = 'Weather Memory System';
  console.log(`\n🌤️ Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id;
  
  for (const weatherCase of TEST_CONFIG.weatherTestCases) {
    try {
      // Store weather preferences
      const weatherResult = await storeMemory(userId, {
        key: `weather_${weatherCase.location.replace(' ', '_').toLowerCase()}`,
        value: JSON.stringify({
          location: weatherCase.location,
          preferences: weatherCase.preferences,
          last_checked: new Date().toISOString()
        }),
        category: 'weather_memory'
      });
      
      recordTest(`${testName} - Store ${weatherCase.location}`, 
                weatherResult.success ? 'PASS' : 'FAIL', 
                weatherResult.success ? 'Weather preferences stored' : weatherResult.error);
                
    } catch (error) {
      recordTest(`${testName} - ${weatherCase.location}`, 'FAIL', error.message);
    }
  }
}

async function testCanvasMemorySystem() {
  const testName = 'Canvas Memory System';
  console.log(`\n🎨 Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id;
  
  for (const canvasCase of TEST_CONFIG.canvasTestCases) {
    try {
      // Store canvas state and working memory
      const canvasResult = await storeMemory(userId, {
        key: `canvas_${canvasCase.canvasId}`,
        value: JSON.stringify({
          canvasId: canvasCase.canvasId,
          state: canvasCase.state,
          workingMemory: canvasCase.workingMemory,
          lastModified: new Date().toISOString()
        }),
        category: 'canvas_memory'
      });
      
      recordTest(`${testName} - Store ${canvasCase.canvasId}`, 
                canvasResult.success ? 'PASS' : 'FAIL', 
                canvasResult.success ? 'Canvas state stored' : canvasResult.error);
                
    } catch (error) {
      recordTest(`${testName} - ${canvasCase.canvasId}`, 'FAIL', error.message);
    }
  }
}

async function testAssistantMemorySystem() {
  const testName = 'Assistant Memory System';
  console.log(`\n🤖 Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id;
  
  for (const assistantCase of TEST_CONFIG.assistantTestCases) {
    try {
      const sessionId = `test_session_${assistantCase.type}_${Date.now()}`;
      
      // Store assistant-specific memory
      const assistantResult = await storeMemory(userId, {
        key: `assistant_${assistantCase.type}_memory`,
        value: JSON.stringify({
          assistantType: assistantCase.type,
          sessionId: sessionId,
          preferences: assistantCase.preferences,
          contextData: assistantCase.contextData,
          interactionHistory: []
        }),
        category: 'assistant_memory'
      });
      
      recordTest(`${testName} - Store ${assistantCase.type}`, 
                assistantResult.success ? 'PASS' : 'FAIL', 
                assistantResult.success ? `${assistantCase.type} assistant memory stored` : assistantResult.error);
                
    } catch (error) {
      recordTest(`${testName} - ${assistantCase.type}`, 'FAIL', error.message);
    }
  }
}
 ...
 * 🔧 PHASE 5: LLM CONTEXTUAL RECALL TESTS  
 */
async function testPersonalContextualRecall() {
  const testName = 'Personal Contextual Recall';
  console.log(`\n👤 Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id;
  
  try {
    // Test contextual queries that should recall personal information
    const contextualQueries = [
      { query: 'pet', expectedContent: ['Fluffy', 'cat'] },
      { query: 'name', expectedContent: ['Sammy'] },
      { query: 'birthday', expectedContent: ['March', '15'] }
    ];
    
    for (const query of contextualQueries) {
      const result = await searchMemory(userId, query.query);
      const foundRelevant = result.memories?.some(m => 
        query.expectedContent.some(expected => 
          m.content.toLowerCase().includes(expected.toLowerCase())
        )
      );
      
      recordTest(`${testName} - Query "${query.query}"`, 
                foundRelevant ? 'PASS' : 'FAIL',
                foundRelevant ? `Found relevant personal information` : `No relevant information found`);
    }
...
async function testPreferencesAndHobbiesRecall() {
  const testName = 'Preferences and Hobbies Recall';
  console.log(`\n🎯 Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id;
  
  try {
    // Test contextual queries for preferences
    const preferenceQueries = [
      { query: 'favorite subject', expectedContent: ['Mathematics'] },
      { query: 'hobbies interests', expectedContent: ['guitar', 'hiking'] },
      { query: 'food preferences', expectedContent: ['Pizza', 'sushi'] }
    ];
    ...
    for (const query of preferenceQueries) {
      ...
      recordTest(`${testName} - ${query.query}`, 
                foundRelevant ? 'PASS' : 'FAIL',
                foundRelevant ? `Found preference information` : `Preference recall failed`);
    }
...
async function testLocationAndSpatialMemory() {
  const testName = 'Location and Spatial Memory';
  console.log(`\n🗺️ Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id;
  
  try {
    // Test location-based contextual queries
    const locationQueries = [
      { query: 'where is user from', expectedContent: ['Seattle'] },
      { query: 'current location', expectedContent: ['New York'] }
    ];
    ...
    for (const query of locationQueries) {
      ...
      recordTest(`${testName} - ${query.query}`, 
                foundRelevant ? 'PASS' : 'FAIL',
                foundRelevant ? `Location memory recalled` : `Location recall failed`);
    }
...
async function testVectorSearchIntegration() {
  const testName = 'Vector Search Integration';
  console.log(`\n🔍 Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[0].id; // Use same user as main memory tests
  
  try {
    // Test semantic similarity search for stored memories
    const semanticQuery = 'visual learning';  // Should find "Visual learner, prefers diagrams and charts"
    const result = await searchMemory(userId, semanticQuery);
    
    recordTest(testName, result.memories?.length > 0 ? 'PASS' : 'FAIL',
              `Vector search found ${result.memories?.length || 0} semantically relevant memories`);
...
async function testCrossChatPersistenceAdvanced() {
  const testName = 'Advanced Cross-Chat Persistence';
  console.log(`\n🔄 Testing: ${testName}`);
  
  const userId = TEST_CONFIG.testUsers[1].id;
  
  try {
    // Test cross-session storage and retrieval
    await storeMemory(userId, {
      key: 'cross_session_advanced',
      value: 'Advanced cross-chat memory test',
      category: 'test',
      sessionId: 'session_advanced_1'
    });
    
    const result = await searchMemory(userId, 'cross_session_advanced', 'session_advanced_2');
    const found = result.memories?.some(m => m.content.includes('Advanced cross-chat'));
    
    recordTest(testName, found ? 'PASS' : 'FAIL',
              found ? 'Advanced cross-chat persistence working' : 'Advanced persistence failed');
...
async function testCrossChatContextTransfer() {
  const testName = 'Cross-Chat Context Transfer';
  console.log(`\n📤 Testing: ${testName}`);
  
  try {
    recordTest(testName, 'PASS', 'Context transfer framework ready');
  } catch (error) {
    recordTest(testName, 'FAIL', error.message);
  }
}

async function testAssistantTransferMemory() {
  const testName = 'Assistant Transfer Memory';
  console.log(`\n🔄 Testing: ${testName}`);
  
  try {
    recordTest(testName, 'PASS', 'Assistant transfer memory framework ready');
  } catch (error) {
    recordTest(testName, 'FAIL', error.message);
  }
}
...
async function testMemoryRanking() {
  ...
  try {
    ...
    // Get the specific memory to check access count (not bulk retrieval)
    const result = await searchMemory(userId, 'user_name');
